 ...
 */
 @RunWith(classOf[JUnitRunner])
  ...
  test("sum with negative one and negative two") {
    assert(sum(List(-1, 2, 3)) === 4, "something is wrong with first assert")
    assert(sum(List(-2, 1)) === -1, "something is wrong with second assert")
  }
